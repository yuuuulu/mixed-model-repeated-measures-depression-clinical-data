...
# package explanation for the unstructured matrix

https://openpharma.github.io/mmrm/main/articles/algorithm.html
...
# 更新模型以包括 base_situation
fit <- mmrm(
  formula = AVAL ~ SEX:TRT01A +TRT01A + SITEID+ RACE + AGE + base_situation + us(AVISITN | USUBJID),
  data = admadrs)
...
# still can not write a good cv code---in fact, just can not run out..
...
# Delete duplicate points in time
admadrs <- admadrs[!duplicated(admadrs[c("USUBJID", "AVISIT")]), ]
...
# Function to perform MMRM on training data and evaluate on test data
cv_mmrm <- function(train_indices, data) {
  # Training data
  ...
  tryCatch({
    ...
    # Note: mmrm doesn't have a standard predict method, so we'll use fitted values
    # This is an approximation for cross-validation purposes
    
    # Get fitted values from the model
    fitted_vals <- fitted(model)
    
    # Create a mapping to match test data with appropriate fitted values
    # This is a simplified approach
    test_data$pred <- NA
    for(i in 1:nrow(test_data)) {
      # Find matching subject/visit in training data
      match_idx <- which(
        train_data$AVISITN == test_data$AVISITN[i] &
        train_data$TRT01A == test_data$TRT01A[i] &
        train_data$SITEID == test_data$SITEID[i] &
        train_data$RACE == test_data$RACE[i] &
        abs(train_data$AGE - test_data$AGE[i]) < 5 &  # Approximate age match
        train_data$base_situation == test_data$base_situation[i]
      )
      
      if(length(match_idx) > 0) {
        # If we find matches, use the average of their fitted values
        test_data$pred[i] <- mean(fitted_vals[match_idx])
      } else {
        # If no exact match, use a more relaxed matching approach
        match_idx <- which(
          train_data$AVISITN == test_data$AVISITN[i] &
          train_data$TRT01A == test_data$TRT01A[i]
        )
        ...
        if(length(match_idx) > 0) {
          ...
          # If still no match, use the overall mean
          test_data$pred[i] <- mean(fitted_vals)
        }
      }
    }
    
    # Calculate MSE
    mse <- mean((test_data$AVAL - test_data$pred)^2, na.rm = TRUE)
    
    return(mse)
  }, error = function(e) {
    # Return a large error value if the model fails
...
admadrs$AVISITN <- factor(admadrs$AVISITN)
admadrs$CHG <- as.numeric(admadrs$CHG)
admadrs$TRT01A <- factor(admadrs$TRT01A)  # 确保治疗变量是因子类型
...
# 比较各治疗组与安慰剂的差异
contrasts <- contrast(emm, method = "trt.vs.ctrl", ref = "Dummy A")
print(contrasts)

# 2. 个体方差分析
subject_variance <- admadrs %>%
  group_by(USUBJID) %>%
  summarize(
    variance = var(AVAL, na.rm = TRUE),
    n_obs = n(),
    treatment = first(TRT01A)
  ) %>%
  filter(n_obs >= 3) # 确保有足够的观测值以获得可靠的方差估计

# 可视化不同治疗组的方差分布
p1 <- ggplot(subject_variance, aes(x = treatment, y = variance)) +
  geom_boxplot() +
  geom_jitter(width = 0.2, alpha = 0.5) +
  labs(title = "The individual variance distribution of each treatment group",
       x = "Treatment Group", y = "Intra-individual variance")
print(p1)

# 检验治疗组之间的方差差异
variance_model <- lm(variance ~ treatment, data = subject_variance)
variance_anova <- anova(variance_model)
print(variance_anova)

# 识别高方差个体（例如，前25%）
high_var_threshold <- quantile(subject_variance$variance, 0.75, na.rm = TRUE)
high_var_subjects <- subject_variance %>%
  filter(variance >= high_var_threshold) %>%
  arrange(desc(variance))
print(high_var_subjects)

# 3. 考虑异质方差的混合模���
var_by_trt_model <- lme(AVAL ~ AVISITN + TRT01A + AVISITN:TRT01A + SITEID + RACE + AGE + base_situation,
                       random = ~1|USUBJID,
                       weights = varIdent(form = ~1|TRT01A),
                       data = admadrs)
var_model_summary <- summary(var_by_trt_model)
print(var_model_summary)

# 4. 移除高方差个体后的MMRM模型
low_var_subjects <- subject_variance %>%
  filter(variance < high_var_threshold) %>%
  pull(USUBJID)

admadrs_filtered <- admadrs %>%
  filter(USUBJID %in% low_var_subjects)

fit_filtered <- mmrm(
  formula = AVAL ~ AVISITN:TRT01A + TRT01A + SITEID + RACE + AGE + base_situation + us(AVISITN | USUBJID),
  data = admadrs_filtered
)

model_summary_filtered <- summary(fit_filtered)
print(model_summary_filtered)

# 计算过滤后的治疗组在各时间点的估计边��均值
emm_filtered <- emmeans(fit_filtered, ~ TRT01A | AVISITN)
print(emm_filtered)
```
...
# 样本生成函数
generate_samples <- function(num_rows, alpha, beta, censor_rate) {
  generate_type2_censored <- function(n, m, alpha, beta) {
    full_times <- bsgof::rbs(n, alpha, beta)
    sorted_times <- sort(full_times)
    observed_times <- append(sorted_times[1:m], rep(sorted_times[m], n - m)) # 将第 m 个值重复 n - m 次（删失率达到指定比例结束实验）
    status <- append(rep(1, m), rep(0, n - m))
    return(list(
      time = observed_times,
      delta = status
    ))
  ...
  # 使用lapply生成样本
  n <- 30  # 每行样本大小
  m <- n * (1 - censor_rate)  # 根据删失率计算失效数量
  samples <- lapply(1:num_rows, function(i) generate_type2_censored(n, m, alpha, beta))
...
bs_mixture_em <- function(t, delta, alpha_0, beta_0) {
  ...
  calculate_gamma <- function(t, alpha, beta) {
    s1 <- 1 - (2 * pnorm(1 / alpha * (sqrt(t / beta) - sqrt(beta / t))) -
                 pinvgauss(t, mean = beta, shape = beta / alpha^2, dispersion = NULL, lower.tail = TRUE, log.p = FALSE))
    s2 <- 1 - pinvgauss(t, mean = beta, shape = beta / alpha^2, lower.tail = TRUE, log.p = FALSE)
    return(s1 / (s1 + s2))
  }
  
  calculate_T_star <- function(t, alpha, beta) {
    tryCatch({
      integrand <- function(x) x * (0.5 * f_1(x, alpha, beta) + 0.5 * f_2(x, alpha, beta))
  ...
  # 初始参数
  current <- list(alpha = alpha_0, beta = beta_0)
  ...
  while (!converged) {
    ...
    epsilon <- sapply(uncensored, calculate_epsilon, current$alpha, current$beta)
    gamma <- sapply(censored, calculate_gamma, current$alpha, current$beta)
    T_star <- sapply(censored, calculate_T_star, current$alpha, current$beta)
    ...
    g <- function(beta) sum1 + (n / 2) * (-sum2 + beta^2 * sum3) / (sum2 + beta^2 * sum3) - n / 2
    beta_new <- uniroot(g, c(0, 1e6), tol = 1e-12)$root
    alpha_new <- sqrt((1 / n) * (sum2 / beta_new + beta_new * sum3 - 2 * n))
...
# 表格函数
analyze_samples <- function(samples, alpha_0, beta_0) {
  # 使用sapply对每行样本运行 EM 算法
  results <- t(sapply(samples, function(sample) {
    result <- bs_mixture_em(sample$time, sample$delta, alpha_0, beta_0)
    c(result$alpha, result$beta)
  }))
  
  # 计算统计量
  alpha_mean <- mean(results[, 1])
  alpha_sd <- sd(results[, 1])
  beta_mean <- mean(results[, 2])
  beta_sd <- sd(results[, 2])
  cov_matrix <- cov(results)
...
beta <- 1.0
alpha_values <- c(2)
censor_rates <- c(0.4,0.6)
alpha_0 <- 1.0
beta_0 <- 0.5

# 使用lapply和sapply组合循环
results_list <- lapply(alpha_values, function(alpha) {
  lapply(censor_rates, function(censor_rate) {
    print(paste("Processing alpha =", alpha, "and censor_rate =", censor_rate))
    samples <- generate_samples(num_rows, alpha, beta, censor_rate)
    result <- analyze_samples(samples, alpha_0, beta_0)
    
    # 提取协方差矩阵的对角线元素（作为协方差）
    covariance <- sum(diag(result$cov_matrix))
    
    # 返回结果
    result_df <- data.frame(
      Alpha = alpha,
      Censoring_Proportion = censor_rate,
      Alpha_Mean = result$alpha_mean,
      Alpha_SD = result$alpha_sd,
...
# 将结果列表转换为数据框
results_table <- do.call(rbind, unlist(results_list, recursive = FALSE))

# 输出结果表格
print(results_table)

# 保存到Excel文件
#file_path <- "results_table.xlsx"
#write.xlsx(results_table, file_path)
```
...
# Install the nleqslv package if you haven't already
...
# Solve the equations
solution <- nleqslv(initial_values, equations)
